            ...
            ?>
                ...
                ?>
                        ...
                        <td class="input-group custom-search-form">
                            ...
                            <input type="number" min="0" max="999"  data-id="<?php echo $result->PROID; ?>"
                            ...
                            <span class="input-group-btn">
                                <a title="Remove Item" class="btn btn-del" id="btnsearch" name="btnsearch"
  ...
  if ($countcart > 0) {
    ...
    if (isset($_SESSION['CUSID'])) {
      ...
      echo '<button type="submit"  name="proceed" id="proceed" class="btn btn-success pull-right">
        ...
        echo '<button type="button" class="btn btn-default check_out signup pull-right" style="background-color: seagreen; color:white; border:none;" onclick="showAlert()">
            Proceed And Checkout
            <i class="fa fa-arrow-right fa-fw"></i>
          </button>';
    }
...
function handleQuantityChange(inputId) {
    ...
    if (currentValue > originalQuantity) {
        ...
        Swal.fire({
            ...
            text: 'The quantity has been adjusted to the maximum available.',
            icon: 'info',
...
function showAlert() {
  Swal.fire({
    ...
    text: 'Please sign in first to proceed with the checkout.',
    icon: 'warning',
    ...
    if (result.isConfirmed) {
      window.location.href = 'onlinecustomer/index.php'; // Redirect to the sign-in page
    }
  });
}
</script>
