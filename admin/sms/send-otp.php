...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    try {
        ...
        $stmt = $conn->prepare("SELECT USERID FROM tbluseraccount WHERE U_CON = ?");
        if (!$stmt) {
        ...
        if ($result->num_rows > 0) {
            ...
            $update_stmt = $conn->prepare("UPDATE tbluseraccount SET OTP = ?, OTP_TIMESTAMP = CURRENT_TIMESTAMP WHERE U_CON = ?");
            if (!$update_stmt) {
                throw new Exception("Database prepare error: " . $conn->error);
            }
