...
$sql = "SELECT id, images, productName, productStock, 
        CASE
        ...
        <div class="app-header-inner">
         ...
         if ($notificationCount > 0) {
            while($row = $result->fetch_assoc()) {
                ...
                echo '</div>';
                echo '</div>';
                echo '<input type="hidden" class="product-id" value="' . $productId . '">';
                echo '</a>';
                echo '</div>';
            }
        } else {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // If productId is present, show the modal and then remove the parameter from the URL
    if (productId) {
        var modal = document.getElementById('editProductModal' + productId);
        if (modal) {
            var modalInstance = new bootstrap.Modal(modal);
            modalInstance.show();
            
            // Remove the productId parameter from the URL without reloading the page
...
function updateUnreadMessageCount() {
    $.ajax({
        url: '<?php echo web_root; ?>admin/theme/get_unread_count.php',
        type: 'GET',
        dataType: 'json',
        success: function(data) {
            var unreadCount = data.unread_count;
            var latestTimestamp = data.latest_timestamp;
            var badge = $('#unread-message-count');
            
            if (unreadCount > 0) {
                badge.text(unreadCount).show();
                
                // You can use the latest timestamp if needed
                console.log("Latest unread message at:", new Date(latestTimestamp).toLocaleString());
                
                // Update tooltip or title with latest message info
                badge.attr('title', `${unreadCount} customer(s) with unread messages. Latest at ${new Date(latestTimestamp).toLocaleString()}`);
            } else {
                badge.hide();
            }
        },
...
setInterval(updateUnreadMessageCount, 3000);
