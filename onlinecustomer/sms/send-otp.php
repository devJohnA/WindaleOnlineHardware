...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    try {
        ...
        $stmt = $conn->prepare("SELECT CUSTOMERID FROM tblcustomer WHERE PHONE = ?");
        if (!$stmt) {
        ...
        if ($result->num_rows > 0) {
            ...
            $update_stmt = $conn->prepare("UPDATE tblcustomer SET OTP = ?, OTP_TIMESTAMP = CURRENT_TIMESTAMP WHERE PHONE = ?");
            if (!$update_stmt) {
                throw new Exception("Database prepare error: " . $conn->error);
            }
